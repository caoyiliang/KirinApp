...
internal static class Utils
{
    ...
    public static string RelativeUrl(this Uri host, string requestUrl, string homePage)
    {
        if (requestUrl.Contains('?'))
            requestUrl = requestUrl.Substring(0, requestUrl.IndexOf('?'));
        var result = host.MakeRelativeUri(new Uri(requestUrl)).ToString();
        if (string.IsNullOrEmpty(result))
            return homePage;
        return result;
    }

    static Lazy<Dictionary<string, string>> mappings = new Lazy<Dictionary<string, string>>(() =>
    {
    ...
    public static Dictionary<string, string> Mappings => mappings.Value;

    public static (bool succesd, string contentType) GetContentType(this string file)
    {
        string? extension = GetExtension(file);
        if (extension == null) return (false, "");

        var value = "";
        if (!Mappings.TryGetValue(extension, out value)) return (false, "");
        return (true, value);
    }
    ...
    public static string GetEmbeddedManifestContent(this IFileProvider fileProvider, string path)
    {
